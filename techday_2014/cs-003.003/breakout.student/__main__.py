...
def fix_bricks(args):
    ...
    To reset the above three 'controller', note that you can treat global variables in a module
    ...
    try:
        assert len(args) == 3, 'Did not provide valid number of arguments'
        assert int(args[1]) > 0 and int(args[2]) > 0, 'Did not provide positive integers are arguments'
